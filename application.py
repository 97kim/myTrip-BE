...
def login_check(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if access_token is not None:
            ...
            g.user = get_user_info(payload["username"])
        else:
            g.username = None
            g.user = None
...
def sign_in():
    ...
    if get_user_info(username_receive) is None:
        return Response(status=401)

    if bcrypt.checkpw(password_receive.encode('utf-8'), result['password']):
        payload = {
            "username": username_receive,
            "exp": datetime.utcnow() + timedelta(seconds=60 * 60 * 24)
        }
        token = jwt.encode(payload, SECRET_KEY, algorithm='HS256')
        return jsonify({'result': 'success', 'token': token, 'now_username': payload['username']})
    else:
        return Response(status=401)
...
def sign_up():
    ...
    if get_user_info(username_receive) is not None:
        return Response(status=423)

    doc = {
        "username": username_receive,  # 아이디
        "password": bcrypt.hashpw(password_receive.encode('UTF-8'), bcrypt.gensalt()),  # 비밀번호
...
def give_popular_bookmarks_id():
    all_bookmarks = list(db.bookmark_popular.find({"username": g.username}, {"_id": False}))
...
def get_bookmark_popular(content_id):
    bookmark_status = bool(
        db.bookmark_popular.find_one({"content_id": content_id, "username": g.username}))
...
def get_bookmark(content_id):
    bookmark_status = bool(db.bookmark.find_one({"content_id": content_id, "username": g.username}))
...
def give_bookmarks_id():
    all_bookmarks = list(db.bookmark.find({"username": g.username}, {"_id": False}))
...
@login_check
def write_trip():
    user_info = db.users.find_one({"username": g.username})
...
def save_profile():
    ...
    db.users.update_many({'username': g.username}, {'$set': new_doc})
    db.trips.update_many({'username': g.username}, {'$set': new_doc})
    db.comments.update_many({'username': g.username}, {'$set': new_doc})
...
def show_comments(trip_id):
    all_comments = list(
        db.comments.find({'trip_id': ObjectId(trip_id)}, {'trip_id': False}).sort('date', -1))

    for comments in all_comments:
        comments['_id'] = str(comments['_id'])

    return jsonify({'all_comments': all_comments, 'now_user': g.username})
...
def delete_comment(trip_id):
    ...
    db.comments.delete_one(
        {'trip_id': ObjectId(trip_id), '_id': comment_id_receive, 'username': g.username})

    return jsonify({'result': 'success'})


@application.route('/own', methods=['POST'])
@login_check
def own_check():
    trip_id_receive = request.form['trip_id']

    owner = db.trips.find_one({'_id': ObjectId(trip_id_receive)})

    owner['_id'] = str(owner['_id'])

    return jsonify({'owner': owner, 'now_user': g.username})
